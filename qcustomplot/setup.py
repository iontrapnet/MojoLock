...
WIN = (os.name == 'nt')
CONDA = 'E:/conda3/Library/' if WIN else os.getenv('HOME')+'/conda3/' 

DEFAULT_QMAKE = CONDA + 'bin/qmake' + ('.exe' if WIN else '')
DEFAULT_MAKE = 'nmake' if WIN else 'make'
DEFAULT_QT_INCLUDE = CONDA + 'include/qt'
...
class MyBuilderExt(build_ext):
    ...
    def __build_qcustomplot_library(self):
        qcustomplot_static = join(self.build_temp, 'release/qcustomplot.lib' if WIN else 'libqcustomplot.a')
    ...
    def build_extensions(self):
        ...
        if WIN:
            qcustomplot_ext.library_dirs += [CONDA + 'lib']
